...
stage('Build Docker Image') {
  steps {
    withCredentials([string(credentialsId: 'AWS_REPOSITORY_URL_SECRET', variable: '057320992418.dkr.ecr.ap-south-1.amazonaws.com')]) {
      script {
        docker.build("${AWS_ECR_URL}:0.0.1-SNAPSHOT", "--build-arg JAR_FILE=devOpsDemo-0.0.1-SNAPSHOT.jar .")
      }
    }
  }
}
  ...
  steps {
    withCredentials([string(credentialsId: 'AWS_EXECUTION_ROL_SECRET', variable: 'AWS_ECS_EXECUTION_ROL'),string(credentialsId: 'AWS_REPOSITORY_URL_SECRET', variable: 'AWS_ECR_URL')]) {
      script {
        ...
        sh("/usr/local/bin/aws ecs create-service --cluster ${AWS_ECS_CLUSTER} --service ${AWS_ECS_SERVICE} --task-definition ${AWS_ECS_TASK_DEFINITION}:${taskRevision}")
           //sh("/usr/local/bin/aws ecs create-service --cluster ${AWS_ECS_CLUSTER} --service ${AWS_ECS_SERVICE} --task-definition ${AWS_ECS_TASK_DEFINITION}:${taskRevision}")
      }
    }
  }
